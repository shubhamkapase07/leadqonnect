// Deterministic lead scoring — turns a lead's text + its campaign context into precise,
// explainable Intent / Quality / Match scores (0-100). No randomness: the same input always
// yields the same scores. Used at scan time, and as the fallback when AI qualification is off.
...
// High-intent buying/hiring phrases (someone actively seeking a provider).
const HIGH_INTENT = [
...
export function scoreLead(rawText: string, ctx: CampaignContext): LeadScores {
  ...
  // ---- Match score: how well the lead fits the campaign target ----
  ...
  // Leads are found *by* a keyword, so a base of 40, then reward broader coverage + service/industry fit.
  ...
  // ---- Intent score: how ready-to-buy the post is ----
  ...
  // ---- Quality score: richness & specificity of the lead ----
  const wordCount = text.split(/\s+/).filter(Boolean).length;
  let lengthScore = wordCount < 12 ? 42 : wordCount < 40 ? 55 + (wordCount - 12) * 0.7 : Math.min(78 + (wordCount - 40) * 0.2, 92);
